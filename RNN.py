...
from keras.layers import Dense,Embedding,recurrent,Dropout
...
def rnn_model(X_train,Y_train,X_test,Y_test):
    ...
    model=Sequential()
    model.add(RNN(units=20,input_shape=(X_train.shape[1],X_train.shape[2])))
    model.add(Dropout(.2))
...
if __name__=='__main__':#都是两个杠
    ...
    train_Y,trainPredict,test_Y,testPredict=rnn_model(X_train,Y_train,X_test,Y_test)
