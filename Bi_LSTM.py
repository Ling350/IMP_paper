...
def calc_R(real,predict):
    fenzi=0
    fenmu=0
    mean_real = sum(real[:, 0]) / len(real[:, 0])
    mean_predict=sum(predict [:, 0]) / len(predict[:, 0])
    for i in range(len(predict[:, 0])):
        fenzi+=(real[:, 0][i]-mean_real)*(predict [:, 0][i]-mean_predict)
        fenmu+=(real[:, 0][i]-mean_real)*(predict [:, 0][i]-mean_predict)*(real[:, 0][i]-mean_real)*(predict [:, 0][i]-mean_predict)
    R = fenzi / math.sqrt(fenmu)
    return R

def calc_Theilu(real,predict):
    fenzi=mean_squared_error(real[:,0],predict[:,0])
    fenmu1=0
    fenmu2=0
    for i in range(len(predict[:,0])):
        fenmu1+=real[:, 0][i]*real[:, 0][i]
        fenmu2+=predict [:, 0][i]*predict [:, 0][i]
    fenmu=math.sqrt(fenmu1/len(predict[:, 0]))+math.sqrt(fenmu2/len(predict[:, 0]))
    Theilu=fenzi/fenmu
    return Theilu
...
def evaluate(real,predict):
    ...
    R=calc_R(real,predict)
    Theilu=calc_Theilu(real,predict)

    return RMSE,MAE,MAPE,R,Theilu


def bi_lstm_model(X_train,Y_train,X_test,Y_test):
    #X_train.shape:687 15 5
    model=Sequential()
    model.add(Bidirectional(LSTM(units=20,input_shape=(X_train.shape[1],X_train.shape[2]))))
    model.add(Dense(1,input_dim=(days,20),activation='hard_sigmoid'))
...
if __name__=='__main__':#都是两个杠
    ...
    train_Y,trainPredict,test_Y,testPredict=bi_lstm_model(X_train,Y_train,X_test,Y_test)
    ...
    #
    RMSE,MAE,MAPE,R,Theilu=evaluate(test_Y,testPredict)
    print(RMSE,MAE,MAPE,R,Theilu)
